...
def document_identity(
    ...
    *,
    include_near_fingerprint: bool = False,
) -> DocumentIdentity:
    """Build the enabled document identities without paying for disabled checks.

    SimHash is intentionally opt-in: its Python token/bit loop is much more
    expensive than the exact and normalized hashes, and Stage 2 has near
    duplicate detection disabled in the production corpus recipe.
    """
    ...
    return DocumentIdentity(
        ...
        near_fingerprint=simhash64(normalized) if include_near_fingerprint else None,
    )
